...
class myMLP(object):
    def __init__(self, rng, input, n_in, n_hiddens, n_out, activation):
        ...
        which the datapoints lie
        :type n_hiddens: list
        :param n_hidden: number of hidden units for every hidden layer
        :type n_out: int
        :param n_out: number of output units, the dimension of the space in
        which the labels lie
        """
        ...
        inp_dim = n_in
        n_input = input
        self.hiddenLayers = [None] * len(n_hiddens)
        for i in range(len(n_hiddens)):
        	...
        	inp_dim = n_hiddens[i]
        	n_input = self.hiddenLayers[i].output

        # The logistic regression layer gets as input the hidden units
        # of the hidden layer
        self.logRegressionLayer = LogisticRegression(
            input=self.hiddenLayers[len(n_hiddens) - 1].output,
            n_in=n_hiddens[len(n_hiddens) - 1],
            n_out=n_out
        )
        ...
        # be small
    	self.L1 = sum(
    		[abs(hiddenlayer.W).sum() for hiddenlayer in self.hiddenLayers]
    		) + abs(self.logRegressionLayer.W).sum()

        # square of L2 norm ; one regularization option is to enforce
        # square of L2 norm to be small
        self.L2_sqr = sum(
        	[(hiddenlayer.W**2).sum() for hiddenlayer in self.hiddenLayers]
        ...
        self.params = reduce(lambda x,y:x + y, 
        	[hiddenLayer.params for hiddenLayer in self.hiddenLayers]) + self.logRegressionLayer.params
        # end-snippet-3
...
def test_mlp(activation = T.tanh,learning_rate=0.01, L1_reg=0.00, L2_reg=0.0001, n_epochs=1000,
             batch_size=20, n_hiddens=[500,600], verbose=True):
    """
    ...
    epoch = 0
    done_looping = False
    print ("... create validation error set")
    ...
    while (epoch < n_epochs) and (not done_looping):
        ...
        for minibatch_index in range(n_train_batches):
            ...
            if (iter + 1) % validation_frequency == 0:
                ...
                average_val_losses.append(this_validation_loss)
